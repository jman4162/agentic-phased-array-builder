...
To contextualize the agent's capabilities, Table~\ref{tab:comparison}
compares four design exploration methods on the same 3-variable problem
(maximize EIRP subject to cost~$< \$10{,}000$ and SNR~$> \SI{15}{\deci\bel}$)
with approximately 50~evaluations each. LHS achieves the highest EIRP
through uniform space coverage, while differential evolution
underperforms at this budget due to its population-based overhead on
the mixed integer-continuous search space. All three conventional
methods require problem-specific setup code and a fixed parameterization.
The LLM agent's unique value lies not in optimization speed but in its
ability to orchestrate cross-domain workflows (FEM, pattern, system
tools in a single session), adapt the design topology mid-run (e.g.,
switching taper functions), and produce interpretable reasoning---none
of which traditional optimizers express.

\begin{table}[!t]
  \centering
  \caption{Method Comparison (50-eval budget, 3 variables)}
  \label{tab:comparison}
  \begin{tabular}{lcccc}
    \toprule
    & Random & LHS & DE & Agent \\
    \midrule
    Best EIRP (dBW) & 38.7 & 40.7 & 38.5 & --- \\
    Feasible (\%)   & 60   & 64   & 51   & --- \\
    Setup code      & Yes  & Yes  & Yes  & None \\
    Topology change & No   & No   & No   & Yes \\
    Explains design & No   & No   & No   & Yes \\
    \bottomrule
  \end{tabular}
\end{table}

The open-source APAB toolkit, all scripts, and generated figures are
available at~\cite{apab2026}.
